package com.opustech.bookvan.ui.adapters;

import androidx.fragment.app.Fragment;
import androidx.fragment.app.FragmentManager;
import androidx.fragment.app.FragmentPagerAdapter;

import com.opustech.bookvan.ui.fragments.admin.bookings.BookingsConfirmedAdminFragment;
import com.opustech.bookvan.ui.fragments.admin.bookings.BookingsHistoryAdminFragment;
import com.opustech.bookvan.ui.fragments.admin.bookings.BookingsPendingAdminFragment;

public class BookingsAdminViewPagerAdapter extends FragmentPagerAdapter {

    public BookingsAdminViewPagerAdapter(FragmentManager fm) {
        super(fm);
    }

    @Override
    public Fragment getItem(int position) {
        Fragment fragment = null;
        if (position == 0) {
            fragment = new BookingsConfirmedAdminFragment();
        } else if (position == 1) {
            fragment = new BookingsPendingAdminFragment();
        } else if (position == 2) {
            fragment = new BookingsHistoryAdminFragment();
        }
        return fragment;
    }

    @Override
    public int getCount() {
        return 2;
    }

    @Override
    public CharSequence getPageTitle(int position) {
        String title = null;
        if (position == 0) {
            title = "Confirmed Booking";
        } else if (position == 1) {
            title = "Pending Booking";
        } else if (position == 2) {
            title = "Booking History";
        }
        return title;
    }
}
